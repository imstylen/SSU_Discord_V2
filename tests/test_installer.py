...
def bash(script, tmp_path, **environment):
    ...
    env = os.environ.copy()
    env.update(INSTALLER_SCRIPT=(ROOT / "install.sh").as_posix(),
               TEST_PYTHON=Path(sys.executable).as_posix(),
               CALL_LOG=(tmp_path / "calls").as_posix(), SSH_CONNECTION="", **environment)
    # Only explicitly called, mocked functions run; sourcing never invokes main.
    return subprocess.run([BASH, "-c", 'source "$INSTALLER_SCRIPT"\n' + script],
...
@pytest.mark.parametrize("changes", [
    {"APP_URL": "http://localhost:8010"}, {"APP_URL": "https://127.0.0.1"},
    {"APP_URL": "https://members.ssu-apps.link:444"},
    {"APP_URL": "https://members.ssu-apps.link:secret"},
    {"APP_URL": "https://bad{host.ssu-apps.link"},
    {"DATABASE_URL": "sqlite:///../outside.db"}, {"DATABASE_URL": "sqlite:///:memory:"},
    {"DATABASE_URL": "postgresql://name:private@host/db"},
    {"SESSION_COOKIE_SECURE": "false"}, {"ADMIN_PASSWORD": "bad"},
    {"DISCORD_BOT_TOKEN": ""}, {"DISCORD_CLIENT_ID": "not-a-snowflake"},
    {"DISCORD_VERIFIED_ROLE_ID": "456"},
    {"DISCORD_INVITE_URL": "https://discord.gg/replace-me"},
    {"SMTP_HOST": "mail.example.com"}, {"SMTP_FROM": "community@example.com"},
...
def test_env_is_not_executed_as_shell(deployment):
    _, target, write = deployment
    metadata = validate_environment(write(DISCORD_CLIENT_SECRET="$(touch should-not-exist)"), target)
    assert metadata["hostname"] == "members.ssu-apps.link"
...
def test_installer_backup_captures_wal_without_modifying_source(tmp_path):
    ...
    with sqlite3.connect(database) as connection:
        ...
        with sqlite3.connect(result) as backup:
            assert backup.execute("SELECT email FROM members").fetchone()[0] == "sample@ssu-apps.link"
...
@pytest.mark.parametrize("arguments", ["--bad-option", "--ssh-port", "--ssh-port 0",
                                       "--ssh-port 65536", "--ssh-port bad"])
...
def test_ssh_sources_and_rules_precede_enable(tmp_path):
    result = bash(r'''
parse_args --ssh-port 002222 --ssh-port 2200
...
''', tmp_path, OWN_STATUS="0" if own else "1")
...
def test_rerun_env_is_preserved_and_conflicts_stop(deployment, tmp_path, same):
    ...
    result = bash('SOURCE_DIR="$SOURCE"; INSTALL_DIR="$TARGET"; check_env_conflict', tmp_path,
...
''', tmp_path, DB_FILE=database.as_posix(), TARGET=tmp_path.as_posix())
    assert result.returncode == 0, result.stderr
    lines = (tmp_path / "calls").read_text().splitlines()
...
def test_failed_caddy_validation_restores_both_files(tmp_path):
    work = tmp_path / "work"
    work.mkdir()
    ...
    # Supply a Python shim only for the helper, and mock all privileged commands.
    result = bash(r'''
...
caddy() { return 1; }
systemctl() { printf '%s\n' "$*" >> "$CALL_LOG"; }
# The helper path is normally in the installed venv; create a candidate import
# through a mock executable instead of accessing the real /opt or /etc.
configure_caddy
''', tmp_path, FIXTURE_WORK=work.as_posix(), FIXTURE_ROOT=root.as_posix(),
                  FIXTURE_SITE=site.as_posix())
    # This harness is completed by the reusable helper shim below.
    assert result.returncode != 0
